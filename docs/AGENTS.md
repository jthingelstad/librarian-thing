...
- **This is the spec, Jamie-authored.** Agents may *seed* or *consolidate* these docs from canonical
  sources (the persona prompts, the per-app AGENTS.md, the renderers), but **Jamie owns the voice**
...
- **Distinct from the per-app `AGENTS.md`s** (e.g. `apps/librarian/AGENTS.md`): those are

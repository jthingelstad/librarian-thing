...
- **Answer eval.** `lambda/scripts/eval-answers.mjs` (`npm run eval:answers` in `lambda/`) is the end-to-end answer-quality harness for the live `/chat` loop: it signs in with a real magic sign-in code (fetched over Fastmail JMAP, `FASTMAIL_JMAP_TOKEN`), runs eight archetype questions (synthesis, temporal, exact quote, aggregation, absence, photos, reading history, references), grades each answer with the fast Bedrock model plus deterministic contains-checks, and deletes every conversation it created. `.github/workflows/answer-eval.yml` runs it weekly (Mondays), gated on the `ANSWER_EVAL_ENABLED` repo variable. Each run sends a real email and spends real rate limit - don't loop it.

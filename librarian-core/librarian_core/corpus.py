...
GENERATED_NOTICE_RE = re.compile(
    r"^<!-- Generated by (?:scripts/content|pipeline/content/content)\.py.*?-->\s*", re.S
)
FRONTMATTER_RE = re.compile(r"^---\s*\n(.*?)\n---\s*\n?", re.S)
TEMPLATE_LEAK_RE = re.compile(
    r"({%\s*|{{\s*|buttondown-editor-mode|Generated by (?:scripts/content|pipeline/content/content)\.py)",
    re.I,
)
...
def detect_topics(subject: str, body: str, limit: int = 6) -> list[str]:
    ...
    for topic, keywords in TOPIC_KEYWORDS.items():
        ...
        if score >= minimum:
            counts[topic] = score
    return [
        topic for topic, _ in sorted(counts.items(), key=lambda item: (-item[1], item[0]))[:limit]
    ]
...
def time_sensitivity(subject: str, body: str) -> str:
    ...
    if any(
        term in text
        for term in ("today", "this week", "beta", "launch", "announced", "now available")
    ):
    ...
    if support_data:
        ...
        if past:
            ...
            for org in past:
                ...
                amount_str = (
                    f" (~${amount:,.0f} raised)" if isinstance(amount, (int, float)) else ""
                )
...
def chunk_id(issue_number: Any, section: str, index: int, text: str) -> str:
    digest = hashlib.sha256(
        f"{issue_number}\0{section}\0{index}\0{text}".encode("utf-8")
    ).hexdigest()
    return digest[:16]
...
def build_corpus(
    ...
    years_active = 0
    issue_years = [
        parse_year(issue.get("publish_date")) for issue in issues if issue.get("publish_date")
    ]
    ...
    *,
    ...
    def add(text: str, link_url: str) -> None:
        ...
        if (
            ...
            and domain not in _BLOG_INTERNAL_DOMAINS
            and _is_thingelstad_domain(domain)
        ):
    ...
    *,
    ...
    for text, link_url in _markdown_html_links(notes):
        ...
        if domain in {"thingelstad.com", "www.thingelstad.com", "micro.thingelstad.com"}:
            ...
            target_source_kind = "site"
        link_kind = (
            "internal" if domain == "another.thingelstad.com" or target_source_kind else "external"
        )
        records.append(
            {
                "source_kind": "podcast",
...
def build_podcast_corpus(podcast_dir: Path = PODCAST_DIR) -> dict[str, Any]:
    ...
    for path in sorted(podcast_dir.glob("*.json")):
        ...
        episode_links = _podcast_show_note_links(
            episode, notes, subject=subject, publish_date=publish_date
        )
        ...
        if notes:
            episode_sections.append(
                {
                    "name": "Show notes",
                    "word_count": len(words(notes)),
                    "content_kind": "podcast_notes",
                }
            )
        episodes.append(
            {
...
def fetch_bedrock_embeddings(
    ...
    if len(embeddings) != len(inputs):
        raise RuntimeError(
            f"Bedrock returned {len(embeddings)} embeddings for {len(inputs)} inputs"
        )
    return embeddings


def add_bedrock_embeddings(
    corpus: dict[str, Any], model: str, dimensions: int, batch_size: int = 96
) -> None:
    load_dotenv()
